...
class ModelBuild:
    ...
    def __init__(
        ...
        if self.model_type not in ["GBC", "LR"]:
            raise ValueError(
                "model_type must be 'GBC' for GradientBoostingClassifier or 'LR' for LogisticRegression."
            )
    ...
    def transform_data(self):
        ...
        all_numerical = self.X_train.select_dtypes(
            include=[np.number]
        ).columns.to_list()
        ...
        numerical_features = [
            col
            for col in all_numerical
            if col not in binary_features + self.log_features
        ]
        ...
        logging.info(
            f"Log-Transformed Features ({len(self.log_features)}): {self.log_features}"
        )
        logging.info(
            f"Binary (0/1) Features ({len(binary_features)}): {binary_features}"
        )
        # ----------------------------------------------------
        ...
        categorical_transformer = ImbPipeline(
            steps=[
                (
                    "imputer",
                    SimpleImputer(strategy="constant", fill_value="missing_cat"),
                ),
        ...
        binary_transformer = ImbPipeline(
            steps=[("imputer", SimpleImputer(strategy="constant", fill_value=0))]
        )

        log_transformer = ImbPipeline(
            steps=[
                ("imputer", SimpleImputer(strategy="constant", fill_value=0)),
                (
                    "log",
                    FunctionTransformer(
                        safe_log_transform, feature_names_out="one-to-one"
                    ),
                ),
                ("std_scaler", StandardScaler()),
            ]
        ...
        if self.model_type == "GBC":
            ...
            estimator = GradientBoostingClassifier(
                random_state=42, subsample=0.80, max_features="sqrt"
            )
            ...
            estimator = LogisticRegression(
                random_state=42, solver="saga", max_iter=5000
            )  # Use saga for L1/L2 and large data
            pipeline = ImbPipeline(
                steps=[
                    ("preprocessor", preprocessor),
                    ("smote_tomek", SMOTETomek(random_state=42)),
            ...
            param_grid = {
                **smote_params,
                ...
                "LR__l1_ratio": (
                    [0.1, 0.5, 0.9]
                    if "elasticnet" in ["l1", "l2", "elasticnet"]
                    else [None]
                ),  # For elasticnet
            }
        ...
        stratified_cv = StratifiedKFold(
            n_splits=self.splits, shuffle=True, random_state=42
        )
    ...
    def evaluate_model(self):
        ...
        if self.best_estimator is None:
            logging.error(
                "Model must be trained before evaluation. Run run_model_pipeline() first."
            )
            return
        ...
        # Metric Calculation Loop: Use the imported utility function
        results = {}
        for name, (X_data, y_true) in sets.items():
            logging.info(f"Calculating metrics for {name} set...")
            results[name] = calculate_binary_metrics(
                X=X_data, y_true=y_true, model=self.best_estimator
            )

        # Output Results (Adjusted to match the utility function's output keys)
        print("\n--- PERFORMANCE SUMMARY ---")
    ...
    def save_results_json(
        self, results_dict: dict, filename: str = "model_metrics.json"
    ):
        ...
        try:
            ...
            logging.info(f"Evaluation metrics successfully saved to {full_path}")
    ...
    def save_model_artifact(self, filename: str = "full_model_artifact.pkl"):
        ...
        try:
            ...
            logging.info(f"Full model artifact successfully saved to {full_path}")
        except Exception as e:
            logging.error(f"Failed to save model artifact file: {e}")

    def run_model_pipeline(self, base_output_dir: str = "model_runs"):
        """
        ...
        logging.info(
            f"Current Experiment ID: {self.current_experiment_id} | Timestamp: {self.timestamp}"
        )

        # Define and Create the Experiment Output Directory
        self.output_dir = os.path.join(base_output_dir, run_prefix)
        os.makedirs(self.output_dir, exist_ok=True)
        logging.info(f"Saving artifacts to directory: {self.output_dir}")
        ...
        if results_dict:
            ...
            self.save_results_json(
                results_dict=results_dict, filename=f"{run_prefix}_metrics.json"
            )

        # Save the best fitted pipeline
        self.save_model_artifact(filename=f"{run_prefix}_artifact.pkl")
